...
for step_idx in range(config.environment.max_periods):
    ...
    record = {
        'year': year,
        # CAQ市场价格（B类场景必有）
        'caq_price': float(infos['caq_price'][0].item()),
        # 政策指标
        ...
        # CAQ交易指标
        'num_caq_buyers': int(infos['num_caq_buyers'][0].item()),
        'num_caq_sellers': int(infos['num_caq_sellers'][0].item()),
        'caq_volume': float(infos['caq_volume'][0].item()),
        # 利润统计
...
df_market.to_csv('B_market.csv', index=False)

df_agents = agent_df
df_agents.to_csv('B_agents.csv', index=False)
